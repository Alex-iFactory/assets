...
class Manager
{
    ...
    /**
     * Absolute path to the private directory of your App (APPROOT).
     * Required if you enable the pipeline.
     * No trailing slash!.
     * @var string
     */
    protected $private_dir;
	 ...
	 * Relative to your assets directory ('assets_dir').
	 * No trailing slash!.
	 * @var string
	 */
	...
	public function config(array $config)
	{
        ...
        // Set private dir
        if(isset($config['private_dir']))
            $this->private_dir = $config['private_dir'];

        // Pipeline requires private dir
        if($this->pipeline and ! is_dir($this->private_dir))
            throw new Exception('edencoder/assets: Private dir not found');
		...
		if($this->pipeline and ! is_dir($this->public_dir))
			throw new Exception('edencoder/assets: Public dir not found');
        ...
        if(isset($config['assets_dir']))
            $this->assets_dir = $this->public_dir.$config['assets_dir'];

		// Set custom CSS directory
		if(isset($config['css_dir']))
			$this->css_dir = $this->assets_dir.$config['css_dir'];

		// Set custom JavaScript directory
		if(isset($config['js_dir']))
			$this->js_dir = $this->assets_dir.$config['js_dir'];
	...
	protected function pipeline(array $assets, $extension, $subdirectory, Closure $minifier)
	{
		...
		$relative_path = DIRECTORY_SEPARATOR . trim($this->assets_dir, '/') . DIRECTORY_SEPARATOR . trim($subdirectory, '/') . DIRECTORY_SEPARATOR . trim($this->pipeline_dir, '/') . DIRECTORY_SEPARATOR .$file;
		$absolute_path = DIRECTORY_SEPARATOR . trim($this->public_dir, '/') . $relative_path;
	...
	protected function gatherLinks(array $links)
	{
		...
		foreach($links as $link)
		{
			...
			{
				if (is_file($this->private_dir . DIRECTORY_SEPARATOR . $link)) {
					$link = $this->private_dir . DIRECTORY_SEPARATOR . $link;
				}else{
					if('js' === substr($link, 0, 2)) {
						$link = substr($link, 3);
					}
					if('css' === substr($link, 0, 3)) {
						$link = substr($link, 4);
					}
				}
			}
	...
	public function addDir($directory, $pattern = self::DEFAULT_REGEX)
	{
		...
		if( ! is_dir($this->private_dir))
			throw new Exception('edencoder/assets: Public dir not found');

		// Get files
		$files = $this->rglob($this->private_dir . DIRECTORY_SEPARATOR . $directory, $pattern, $this->private_dir);
