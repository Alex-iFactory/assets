...
The library is no longer framework agnostic, it is built and should work well with any naked PHP application.
...
If your assets have no extension and autodetection fails, then just use canonical functions *(they accept an array of assets too)*
	...
	Assets::addCss('/bundle/assets/css/asset.css');
	Assets::addJs('/bundle/assets/js/asset.js');
...
All methods that don't generate output will accept chaining:
	...
	Assets::reset()->add('collection')->addJs('/bundle/assets/js/file.js')->css();
